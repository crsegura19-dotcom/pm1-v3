...
export function createCombatThread(title, category) {
  ...
  return {
    ...
    decreePrograms: [],         // {id, texts, startDate, durationDays}
    openCount: 1,
...
// El programa avanza solo con el paso de los días — son afirmaciones para
// repetirse mentalmente, no una checklist. Forzar un marcado manual no
// aportaba nada real, así que el progreso es puramente temporal.
...
export function decreeProgramProgress(program) {
  return Math.min(100, Math.round((decreeDayNumber(program) / program.durationDays) * 100));
}
