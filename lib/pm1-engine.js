...
export const DECREE_LIBRARY = {
  procrastination: [
    ...
    "Termino lo que empiezo, un paso a la vez.",
    "El momento de actuar es ahora, no cuando me sienta listo.",
  ],
  emotional_regulation: [
    ...
    "Puedo estar en calma incluso en medio del caos.",
    "Mis emociones me informan, no me controlan.",
  ],
  avoidance: [
    ...
    "Doy la cara a lo que me incomoda, poco a poco.",
    "Cada vez que no evito, me hago más fuerte.",
  ],
  external_validation: [
    ...
    "Actúo desde mis propios valores, no desde la mirada ajena.",
    "Puedo estar en desacuerdo con otros y seguir en paz conmigo.",
  ],
  perfectionism: [
    ...
    "Lo suficientemente bueno es, de hecho, suficiente.",
    "Me permito equivocarme mientras aprendo.",
  ],
  immediate_reward: [
    ...
    "Invierto en quien quiero ser, no solo en cómo me siento ahora.",
    "Mi disciplina de hoy es un regalo para mi yo futuro.",
  ],
  rejection_avoidance: [
    ...
    "Me muestro tal como soy, y eso ya es suficiente.",
  ],
  emotional_disconnection: [
    ...
    "Mi cuerpo y mis emociones son aliados, no enemigos.",
  ],
  automatic_routine: [
    ...
    "Vivo con propósito, no en piloto automático.",
    "Cada día es una oportunidad nueva de elegir distinto.",
  ],
  excessive_control: [
    ...
    "Puedo soltar y seguir estando bien.",
    "La vida tiene espacio para lo inesperado, y yo también.",
  ],
  self_sabotage: [
    ...
    "No necesito destruir lo que construyo para sentirme yo mismo.",
    "Sostengo mi éxito con la misma fuerza con la que lo persigo.",
  ],
  social_comparison: [
    ...
    "Soy suficiente exactamente como soy hoy.",
    "Mi valor no se compara, se vive.",
  ],
  catastrophizing: [
    ...
    "No todo pensamiento merece mi confianza total.",
    "Puedo sostener la incertidumbre sin anticipar el desastre.",
  ],
  people_pleasing: [
    ...
    "Mi paz importa tanto como la de los demás.",
  ...
  chronic_indecision: [
    ...
    "Actuar y ajustar es mejor que quedarme quieto esperando certeza.",
    "Confío en que puedo corregir el rumbo si hace falta.",
  ],
  overcommitment: [
    ...
    "Mi energía es finita y la administro con cuidado.",
    "Puedo cumplir bien con menos, en vez de mal con todo.",
  ],
  self_isolation: [
    ...
    "Abrirme a otros también es una forma de fuerza.",
    "No tengo que atravesar esto solo.",
  ],
  general: [
...
export function startDecreeProgram(profile, threadId) {
  ...
  const program = {
    id: uid("d_"),
    texts: shuffleArray(pool).slice(0, 3),
...
// Evita marcar dos veces el mismo turno el mismo día — antes se podía pulsar
// "Marcar mañana" repetidamente sin ningún efecto visible, lo que parecía un
// botón roto. Ahora, si ya está marcado hoy, no hace nada (la UI lo deshabilita).
export function logDecreeCheckin(profile, threadId, programId, slot) {
  ...
  const decreePrograms = thread.decreePrograms.map((p) => {
    ...
    const already = p.checkins.some((c) => c.slot === slot && new Date(c.date).toDateString() === todayKey);
    if (already) return p;
    return { ...p, checkins: [...p.checkins, { date: new Date().toISOString(), slot }] };
  });
  return { ...profile, threads: { ...profile.threads, [threadId]: { ...thread, decreePrograms } } };
}

export function isDecreeSlotCheckedToday(program, slot) {
  const todayKey = new Date().toDateString();
  return program.checkins.some((c) => c.slot === slot && new Date(c.date).toDateString() === todayKey);
}

export function decreeDayNumber(program) {
  const start = new Date(program.startDate);
  const now = new Date();
  const days = Math.floor((now - start) / 86400000) + 1;
  return Math.min(program.durationDays, Math.max(1, days));
}
