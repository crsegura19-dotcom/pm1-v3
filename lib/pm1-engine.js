...
export function addPrinciples(profile, threadId, texts) {
  ...
  const newOnes = texts.filter(Boolean).map((t) => ({ id: uid("pr_"), text: t, date: now }));
...
9. Cuando haya un cierre real — al terminar un combate o cuando el usuario comprende algo importante sobre sí mismo — genera entre 1 y 3 Principios PM1 (ver reglas abajo).
